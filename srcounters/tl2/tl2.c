...
#ifndef GVERSIONS_TO_WRITE_IN_FUTURE
    #define GVERSIONS_TO_WRITE_IN_FUTURE 0
#endif
...
GVGenerateWV_GV4 (Thread* Self, vwLock maxv)
{
    ...
    return wv;
#else
    vwLock wv = (uintptr_t) _GCLOCK->inc(2); // increment BY TWO
    wv = max(wv, UNS(maxv) + 2); // probably unneeded
    Self->wv = wv;
    return wv;
#endif
}
...
GVAbort (Thread* Self)
{
#if GVERSIONS_TO_WRITE_IN_FUTURE > 0 && !defined(TL2_ORIGINAL)
    GVGenerateWV(Self, 0); // increment global clock, as if we are a successful writer (with an empty write set). this helps guarantee progress for readers.
#endif
//#if _GVCONFIGURATION != 4
//    vwLock abv = Self->abv;
//    if (abv & LOCKBIT) {
...
TxShutdown ()
{
    ...
    printf("TL2 system shutdown:\n"
           "  GCLOCK=0x%lX Starts=%li Aborts=%li\n"
           "  Overflows: R=%li W=%li L=%li\n",
           (unsigned long) (_GCLOCK->readAccurate()), StartTally, AbortTally,
           ReadOverflowTally, WriteOverflowTally, LocalOverflowTally);
#endif
...
#  endif
            ...
            ASSERT(*(p->LockFor) == (UNS(p)|LOCKBIT));
#if GVERSIONS_TO_WRITE_IN_FUTURE > 0 && !defined(TL2_ORIGINAL)
            vwLock newv = max(wv, max(UNS(p->rdv)+2, Self->rv+2)) + GVERSIONS_TO_WRITE_IN_FUTURE*2;
            *(p->LockFor) = newv;
//            cout<<"Self->rv="<<Self->rv<<" oldv="<<p->rdv<<" newv="<<newv<<endl;
#else
            *(p->LockFor) = wv;
#endif
        }
        ...
        for (p = wr->List; p != End; p = p->Next) {
            ...
            if ((cv & LOCKBIT) && ((AVPair*)(cv ^ LOCKBIT))->Owner == Self) {
                ...
                if (FindFirst(rd, LockFor) != NULL) {
                    vwLock rdv = ((AVPair*)(cv ^ LOCKBIT))->rdv;
                    if (rdv > Self->rv) {
                        Self->abv = cv;
                        return 0;
                    }
...
#else
        CAS(Lock, val, (_GCLOCK->readFast() & ~LOCKBIT));
#endif
        Addr++;
    }
