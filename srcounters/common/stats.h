...
namespace stats_ns {
    ...
    #define MAX_THREAD_BUF_SIZE (1<<24)
    ...
    public:
                ...
                , num_stats(0)
        {
            ...
            assert(sizeof(long long) == DATA_SIZE_BYTES);
...
//            cout<<"appending to id="<<id<<" index="<<index<<" value="<<value<<" at index="<<index<<" result="<<ptr[index]<<endl;
        ...
        bool is_stat_full(const int tid, const stat_id id) {
            return thread_data[tid].size[id] >= thread_data[tid].capacity[id];
        }
...
//            cout<<"finished compute_before_printing()."<<endl;
    ...
    public:
        ...
        stat_metrics<T> * compute_stat_metrics(const stat_id id, enum_aggregation_granularity granularity) {
            if (!already_computed_stats) {
                error("functionality disabled because it is very heavyweight, and is easy to misuse, biasing results. run the print_stat function (or GSTATS_PRINT macro) before calling this, instead.");
            }
            switch (granularity) {
                case TOTAL:
